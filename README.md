# Porta Pac-200 for the Tandy 200

Authentic maze chase in **pure Intel 8085 assembly** for the Tandy 200 portable (Hitachi HD61830 LCD, 240×128). **Porta Pac-200** (Porta CoCo + Model 200).
...
| 4 | `build.py` → `PAC200.CO` + VirtualT guide | Done |

Load address / entry: `0xC800` (decimal **51200**). Runtime `DS` budget: 236 bytes (cap 300). Assembled image: 9014 bytes of machine code plus the 6-byte `.CO` header.
...
- `PAC200.CO` — what the Tandy 200 / VirtualT loads
- `PAC200.LST` — a human-readable listing (address + hex + source), for debugging

The `.CO` header is the standard Kyocera/Tandy 6-byte prefix, little-endian:

| Bytes | Meaning | This project |
|-------|---------|--------------|
| 0–1 | Load address | `C800H` |
| 2–3 | Payload length (not counting the header) | size of the machine code |
| 4–5 | Execution address | `C800H` (`START`) |

## Files

- `build.py` — tiny two-pass 8085 assembler (Phase 4)
- `PAC200.CO` — load this in VirtualT
- `hw_defs.asm` — ports, `ORG 0C800H`, entry/exit, LCD helpers, RAM
...
- `sprite_blit.asm` — 6×6 dirty-rect blitter (one LCD byte per row when X is aligned)
...
### 2. Put `PAC200.CO` in the laptop’s RAM disk

1. Menu **File → Load file from HD**.
2. Choose `PAC200.CO` from this folder.

The name **PAC200.CO** fits the Tandy 6.2 filename limit. It should appear as `PAC200.CO` on the MENU.
...
- `CLEAR 256,51200` tells BASIC: keep 256 bytes for strings, and **do not use RAM from address 51200 (`0xC800`) upward**. That attic is where our program lives.
- `LOADM "PAC200"` copies the machine code from the RAM-disk file to `0xC800` using the header. Use the name **without** `.CO`.

You must `CLEAR` **before** `LOADM`. If you skip it, BASIC can sit on top of `0xC800` and the load will crash the machine.
...
(You still need the `CLEAR 256,51200` first.)

### 5. Controls

| Key | Action |
|-----|--------|
| Arrow keys | Steer (Right wins if two arrows are held) |
...
After BREAK you are back in BASIC. Type `MENU` and Enter to see the file list again. `CALL 51200` restarts without reloading, as long as you did not `CLEAR` away high memory.

### If something goes wrong

| Symptom | Likely cause |
|---------|----------------|
| `OM` (Out of Memory) or BASIC error on `LOADM` | Forgot `CLEAR 256,51200`, or HIMEM is not `51200` |
