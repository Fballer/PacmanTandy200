...
KBD_INIT:
        ...
        OUT     PIO_CMD
        MVI     A,01H                   ; col9 off, no power-down, speaker off
        OUT     PIO_PB
        MVI     A,0FFH
        OUT     PIO_PA
        LXI     H,T200_KSCAN
...
GLGH:   CALL    GHOST_REDRAW
        ...
        CPI     0FFH
        JNZ     GLPACE
        CALL    SPRITES_ERASE           ; death squares still in OLD_XY
...
PM_ANIM:
        LDA     PAC_ANIM
        INR     A
        ANI     0FH                     ; 16 ticks; chew uses low 2 bits (3 open, 1 closed)
        STA     PAC_ANIM
...
;==============================================================================
; GHOST_HIT -- 6x6 sprite boxes overlap (not same-tile).  Inky/Clyde sit on
; the side bunks so their top-left tile often is not Pac's even when the
; sprites overlap.  Eyes ignore; frightened are eaten; house inmates that
; have not left MODE_HOUSE cannot reach Pac (PAC_CAN_GO).
;==============================================================================
GHOST_HIT:
        XRA     A
        STA     CUR_GID
...
GH_EAT: MVI     M,MODE_EATEN
        ...
        LXI     B,GH_FLAGS
        DAD     B
        MVI     M,0                     ; no skip, no queued reverse
        ; Snap to tile origin.  Eaten mid-tile never hit AT_CENTER, so
        ; they kept the old dir, bounced on a wall, and never went home.
        ; Snap to this tile's origin (top-left).  Mid-tile eyes never
        ; hit AT_CENTER; centre+2 can land in a neighbouring wall tile.
        CALL    GHOST_BASE
        MOV     A,M
        CALL    PIX_TO_TX
        CALL    TILE_TO_PX
        ...
        LDA     TMP1
        MOV     M,A
        CALL    GHOST_THINK             ; face the house from this tile
        LDA     GHOST_PTS
        CPI     4
        JNC     GH_SC
        MOV     E,A
...
; Eyes that have stepped into the house box become scatter/chase and
; climb out.  Do not wait for AT_CENTER: PICK_HOME at (9,10) is UP and
; at (9,9) is DOWN, so they reverse every pixel in the well and never
; sit on a centre.  Park on Pinky's floor (not TILE_TO -- a *5 MUL6
; put them on the left pen wall).
EYES_REVIVE:
        XRA     A
        STA     CUR_GID
EV1:    CALL    GHOST_BASE
        ...
        INX     H
        MOV     C,M
        CALL    WALL_RESTORE_BOX        ; wipe pupils before the floor hop
        CALL    GHOST_BASE
        MVI     M,PINKY_X0
        INX     H
        MVI     M,PINKY_Y0
